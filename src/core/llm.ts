...
export class LLM {
   ...
   */
  async process_files(flutterFile: string, arbFile: string, lang: string, packageName: string): Promise<string> {
    const [sysPromptTemplate, humPromptTemplate] = await this._loadPrompt('process_files');
